...
def _quality_cost_table(
    ...
    return {
        "eligible_count": len(route_rows),
        "selected_count": len(selected),
        "llm_call_count": len(selected),
        "cache_hit_count": sum(1 for row in selected if row.get("cache_status") == "hit"),
        ...
        "average_latency_ms": (
            sum(int(row.get("latency_ms", 0) or 0) for row in selected) / len(selected)
            if selected
            else 0.0
        ),
